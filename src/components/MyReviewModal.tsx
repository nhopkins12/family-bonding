...
export function MyReviewModal({ movieId, onClose }: MyReviewModalProps) {
  ...
  const [text, setText] = useState(() => savedReview?.text ?? '')
  const [saveStatus, setSaveStatus] = useState<SaveStatus>('idle')

  useEffect(() => {
    setText(savedReview?.text ?? '')
    setSaveStatus('idle')
  ...
  // Only push to the network ~350ms after the user stops typing, so a burst of
  // keystrokes produces one saved value instead of one write per keystroke. The
  // status label gives reassurance that a long review is actually being saved as
  // you write it, rather than silently trusting a background debounce.
