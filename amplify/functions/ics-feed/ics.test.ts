...
describe('buildIcsCalendar', () => {
  ...
  it('skips a watch with no movie and no date, or an unresolvable movie', () => {
    const ics = buildIcsCalendar(
      [
        { id: 'w1', movieId: undefined, scheduledFor: undefined },
        { id: 'w2', movieId: 'm1' },
  ...
  it('includes an open placeholder as its own event, not a blank day', () => {
    const ics = buildIcsCalendar([{ id: 'vote-2026-09-13', movieId: null, scheduledFor: '2026-09-13' }], movies)
    expect(ics.match(/BEGIN:VEVENT/g)).toHaveLength(1)
    expect(ics).toContain('SUMMARY:Family Bonding: Vote night')
    expect(ics).toContain('DTSTART;VALUE=DATE:20260913')
  })

  it('treats any non-movie row with a date as an open placeholder, regardless of legacy status strings', () => {
    // The app itself no longer distinguishes a "voting" placeholder from any other
    // non-movie kind (e.g. an old "skipped" row) — the feed shouldn't either.
    const ics = buildIcsCalendar([{ id: 'skip-2026-09-13', movieId: null, scheduledFor: '2026-09-13' }], movies)
    expect(ics.match(/BEGIN:VEVENT/g)).toHaveLength(1)
    expect(ics).toContain('SUMMARY:Family Bonding: Vote night')
  })

  it("lists an open placeholder's remaining candidates, ranked by vote count, in its description", () => {
    const ics = buildIcsCalendar(
      [{ id: 'vote-2026-09-13', movieId: null, scheduledFor: '2026-09-13' }],
      movies,
      [
  ...
  it("excludes an already-scheduled or watched movie from an open placeholder's candidates", () => {
    const ics = buildIcsCalendar(
      [
        { id: 'w1', movieId: 'm1', watchedAt: '2026-07-23' },
        { id: 'vote-2026-09-13', movieId: null, scheduledFor: '2026-09-13' },
  ...
  it('escapes commas and semicolons in text fields', () => {
    ...
    const ics = buildIcsCalendar([{ id: 'w1', movieId: 'm1', watchedAt: '2026-07-23' }], commaMovies)
